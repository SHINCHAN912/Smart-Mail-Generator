    ...
    <div className="space-y-6 animate-slide-up">
        ...
        <div className="flex gap-2.5 max-w-md w-full sm:w-auto">
          ...
          <div className="relative">
            <select
              ...
              className="appearance-none pl-3 pr-8 py-1.5 text-xs rounded-xl border border-zinc-200 bg-white focus:outline-none focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500 transition cursor-pointer text-zinc-700 font-semibold"
            >
              {categories.map((cat) => (
      ...
      {filteredHistory.length === 0 ? (
        <div className="text-center py-20 rounded-2xl border border-dashed border-zinc-200 bg-zinc-50/20">
          ...
          <p className="text-sm font-semibold text-zinc-500">No matching emails found</p>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {filteredHistory.map((item) => (
            <div
              key={item.id}
              className="group p-5 rounded-2xl border border-zinc-200/50 bg-white/70 backdrop-blur-md hover:border-indigo-500/30 hover:shadow-lg transition flex flex-col justify-between relative overflow-hidden"
            >
              {/* Highlight bar */}
              ...
              <div>
                {/* Meta details */}
                <div className="flex items-center justify-between gap-2 text-[10px] text-zinc-400 font-semibold mb-3">
                  <div className="flex items-center gap-1.5">
                    <span className="px-2 py-0.5 rounded bg-zinc-100 text-zinc-600 border border-zinc-200/30">
                      {item.category}
                    </span>
                    <span>•</span>
                    <span>{item.tone}</span>
                ...
                {/* Prompt snippet */}
                <div className="my-3 p-2.5 rounded-lg bg-zinc-50/50 border border-zinc-100 text-[11px] text-zinc-500 italic line-clamp-2">
                  &ldquo;{item.prompt}&rdquo;
                </div>

                {/* Truncated draft content */}
                <p className="text-xs text-zinc-600 line-clamp-3 leading-relaxed whitespace-pre-line mb-4 border-l-2 border-zinc-200 pl-3">
                  {item.content}
                </p>
              </div>
              ...
              <div className="pt-3 border-t border-zinc-100 flex items-center justify-between gap-4 mt-auto">
                ...
                <div className="flex items-center gap-2">
                  ...
                  <button
                    ...
                    className={`p-1.5 rounded-lg border transition cursor-pointer hover:bg-zinc-50 ${
                      item.is_saved
